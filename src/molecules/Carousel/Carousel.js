...
export const Carousel = ({ children, gap, mandatoryItemWidth = 0 }) => {
  const [trackIsActive, setTrackIsActive] = useState(false)
  const [multiplier, setMultiplier] = useState(0.35)
  const [sliderWidth, setSliderWidth] = useState(0)
  const [activeItem, setActiveItem] = useState(0)
  const [constraint, setConstraint] = useState(0)
  const [itemWidth, setItemWidth] = useState(mandatoryItemWidth)

  const initSliderWidth = useCallback(width => setSliderWidth(width), [])

  const positions = useMemo(() => children.map((_, index) => -Math.abs((itemWidth + gap) * index)), [
    children,
    itemWidth,
    gap,
  ])
  ...
  useEffect(() => {
    if (isBetweenBaseAndMd) {
      setItemWidth(mandatoryItemWidth || sliderWidth - gap)
      setMultiplier(0.65)
      setConstraint(1)
    }
    if (isBetweenMdAndXl) {
      setItemWidth(mandatoryItemWidth || sliderWidth / 2 - gap)
      setMultiplier(0.5)
      setConstraint(2)
    }
    if (isGreaterThanXL) {
      setItemWidth(mandatoryItemWidth || sliderWidth / 3 - gap)
      setMultiplier(0.35)
      setConstraint(3)
    }
